...
public class ProdutoConsoleController {
    ...
    private void submenuProdutoAdmin(Scanner sc, long id) {
        Produto p;
        try {
            p = produtoService.buscarPorIdComImagens(id); // evita LazyInitializationException
    ...
    while (true) {
        ...
        if ("i".equalsIgnoreCase(opt)) {
            ...
            try {
                ...
                // SEMPRE sair da tela após tentar remover (evita looping)
                ...
                System.out.println("Opção inválida.");
                // volta pra lista novamente (ou troque por 'return' se preferir sair)
            } catch (Exception e) {
                System.out.println("Erro: " + e.getMessage());
                System.out.print("Pressione ENTER para voltar...");
                sc.nextLine();
