...
public class ProdutoConsoleController {
    ...
    private void submenuProdutoAdmin(Scanner sc, long id) {
        ...
        while (true) {
            ...
            if ("1".equals(opt)) {
                ...
                p = produtoService.buscarPorId(p.getId());
    ...
    private void incluirProduto(Scanner sc) {
        ...
        while (true) {
            ...
            String dirOrigem = sc.nextLine().trim();

            System.out.print("Principal (S/N) => ");
            boolean principal = yes(sc);

            
            String caminhoDestino = "/imagens/" + salvo.getId() + (nomeArquivo.startsWith("/") ? nomeArquivo : ("/" + nomeArquivo));
            try {
                produtoService.adicionarImagem(salvo.getId(), caminhoDestino, principal);
    ...
    private void imagensProduto(Scanner sc, long produtoId) {
        while (true) {
            ...
            if ("i".equalsIgnoreCase(opt)) {
                ...
                String caminhoDestino = "/imagens/" + produtoId + (nomeArquivo.startsWith("/") ? nomeArquivo : ("/" + nomeArquivo));
                try {
                    produtoService.adicionarImagem(produtoId, caminhoDestino, principal);
                    // dirOrigem permanece só informativo para o console
                } catch (Exception e) {
                    System.out.println("Erro: " + e.getMessage());
                }
                ...
                try {
                    ...
                    produtoService.removerImagem(produtoId, imgId);
                } catch (NumberFormatException e) {
                    System.out.println("Opção inválida.");
                } catch (Exception e) {
                    System.out.println("Erro: " + e.getMessage());
    ...
    private String extrairNome(String caminho) {
        ...
        // ex.: "/imagens/4/imagem1.jpg" -> "imagem1.jpg"
    }
