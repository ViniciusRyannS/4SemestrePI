...
public class ClienteService {
    ...
    public EnderecoCliente adicionarEndereco(Long idCliente, EnderecoCreateDTO dto) {
        ...
        // Se marcou como padrão de ENTREGA, garante unicidade
        if (e.isPadrao()) {
            garantirApenasUmPadraoEntrega(salvo);
            clientes.save(salvo);
        }

        // Retorna o último endereço persistido (id desc)
        List<EnderecoCliente> atualizados = enderecos.findByClienteIdOrderByIdDesc(idCliente);
        return atualizados.isEmpty() ? e : atualizados.get(0);
    }
